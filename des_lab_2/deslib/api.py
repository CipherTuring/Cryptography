"""
Public API of deslib.

All key/block length validation and the bytes <-> integer conversions
live here; the core (`des_core`) stays pure.
"""
...
def des_encrypt_block(key: bytes, plaintext: bytes) -> bytes:
    """Encrypt a single 8-byte block with an 8-byte DES key."""
...
def des_decrypt_block(key: bytes, ciphertext: bytes) -> bytes:
    """Decrypt a single 8-byte block with an 8-byte DES key."""
    _validate_8_bytes(key, "key")
